...
// Don't duplicate me!
	if ( ! class_exists( 'AvadaReduxFramework_repeater' ) ) {

		/**
		 * Main AvadaReduxFramework_css_layout class
		 *
		 * @since       1.0.0
		 */
		class AvadaReduxFramework_repeater {
			...
			public function render() {
				...
				echo '<div class="avadaredux-repeater-accordion" data-id="' . $this->field['id'] . '">';

				$x = 0;
				if ( isset ( $this->value['avadaredux_repeater_data'] ) && is_array( $this->value['avadaredux_repeater_data'] ) && ! empty ( $this->value['avadaredux_repeater_data'] ) ) {

					$repeaters = $this->value['avadaredux_repeater_data'];
					...
					foreach ( $repeaters as $repeater ) {
						...
						echo '<div class="avadaredux-repeater-accordion-repeater" data-sortid="' . $x . '">';
						echo '<table style="margin-top: 0;" class="avadaredux-repeater-accordion avadaredux-repeater form-table no-border">';
						echo '<fieldset class="avadaredux-field" data-id="' . $this->field['id'] . '">';

						echo '<input type="hidden" name="' . $this->parent->args['opt_name'] . '[' . $this->field['id'] . '][avadaredux_repeater_data][' . $x . '][title]" value="' . esc_attr( $repeater['title'] ) . '" class="regular-text slide-title" data-key="' . $x . '" />';
						...
						echo '<h3><span class="avadaredux-repeater-header">' . $repeater['title'] . ' </span></h3>';
						...
						if ( ! isset( $this->field['static'] ) && empty( $this->field['static'] ) ) {
							echo '<a href="javascript:void(0);" class="button deletion avadaredux-repeaters-remove">' . __( 'Delete', 'avadaredux-framework' ) . ' ' . $this->field['item_name'] . '</a>';
						}
						echo '</div>';
						echo '</fieldset>';
						echo '</table>';
				...
				if ( $x == 0 || ( isset( $this->field['static'] ) && ( $x - 1 ) < $this->field['static'] ) ) {
					...
					while ( $loop > 0 ) {
						echo '<div class="avadaredux-repeater-accordion-repeater">';
						echo '<table style="margin-top: 0;" class="avadaredux-repeater-accordion avadaredux-repeater form-table no-border">';
						echo '<fieldset class="avadaredux-field" data-id="' . $this->field['id'] . '">';
						if ( ! isset( $this->value['avadaredux_repeater_data'][ $x ]['title'] ) ) {
							$this->value['avadaredux_repeater_data'][ $x ]['title'] = "";
						}
						echo '<input type="hidden" name="' . $this->parent->args['opt_name'] . '[' . $this->field['id'] . '][avadaredux_repeater_data][' . $x . '][title]" value="" class="regular-text slide-title" />';

						echo '<h3><span class="avadaredux-repeater-header"> </span></h3>';
						echo '<div>';
				...
				if ( ! isset( $this->field['static'] ) && empty( $this->field['static'] ) ) {
					...
					echo '<a href="javascript:void(0);" class="button avadaredux-repeaters-add button-primary' . $disabled . '" rel-id="' . $this->field['id'] . '-ul" rel-name="' . $this->parent->args['opt_name'] . $this->repeater_values . '[title][]">' . __( 'Add', 'avadaredux-framework' ) . ' ' . $this->field['item_name'] . '</a><br/>';
				}
			...
			public function enqueue() {
				...
				wp_enqueue_script(
					'avadaredux-field-repeater-js',
					apply_filters( "avadaredux/repeater/{$this->parent->args['opt_name']}/enqueue/avadaredux-field-repeater-js", $this->extension_url . 'field_repeater' . $min . '.js' ),
				...
				wp_enqueue_style(
					'avadaredux-field-repeater-css',
					apply_filters( "avadaredux/repeater/{$this->parent->args['opt_name']}/enqueue/avadaredux-field-repeater-css", $this->extension_url . 'field_repeater.css' ),
					array(),
					time(),
					'all'
				);
			...
			public function output_field( $field, $x ) {
				...
				$_field = apply_filters( 'avadaredux-support-repeater', $content, $field, 0 );
				ob_end_clean();
				echo $_field;
			 ...
			 * @since  AvadaRedux_Framework 3.1.5
			 */
			function localize( $field, $value = "" ) {
				...
				if ( isset( $field['fields'] ) && ! empty( $field['fields'] ) ) {
					...
					$var = array(
						'html'     => $var . '<a href="javascript:void(0);" class="button deletion avadaredux-repeaters-remove">Delete </a>',
						'count'    => count( $value ),
						'sortable' => true,
						'limit'    => '',
			...
			private function enqueue_dependencies( $field_type ) {
				$field_class = 'AvadaReduxFramework_' . $field_type;

				if ( ! class_exists( $field_class ) ) {
					$class_file = apply_filters( 'avadaredux-typeclass-load', AvadaReduxFramework::$_dir . 'inc/fields/' . $field_type . '/field_' . $field_type . '.php', $field_class );
